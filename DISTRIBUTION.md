...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\publish-release.ps1 -Version 1.3.1
```

게시 스크립트는 빌드, 배포 입력 검사, ZIP 생성, SHA-256 생성, GitHub 업로드를 순서대로 비대화형 실행합니다. `vendor` 입력이나 빌드 산출물이 없으면 Release를 만들지 않고 오류로 종료합니다.

게시 스크립트는 다음 두 자산을 `v1.3.1` Release에 올립니다.

- `Lawgivers-II-Control-1.3.1.zip`
- `SHA256SUMS.txt`
...
특정 버전을 설치하려면 마지막 호출에 `-Version v1.3.1`을 추가합니다.

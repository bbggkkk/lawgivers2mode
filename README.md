...
- 게임의 기존 인물 능력치 창과 정당 구성원 목록에 직접 통합되는 `CHEAT · 모두 최대` 버튼
...
별도의 대상 선택 메뉴를 만들지 않습니다. 싱글플레이 저장 게임에서 게임이 현재 표시하는 인물 또는 정당을 그대로 대상으로 사용합니다. 게임의 TextMeshPro 한글 폰트, 목록 레이아웃과 회색 패널 스타일을 재사용하며 주황색 표시로 치트 기능임을 구분합니다.

- `인물 능력치`: 기존 능력치 행 아래의 `CHEAT · 모두 최대`로 현재 인물 최대화
- `정당 구성원`: 기존 오른쪽 구성원 목록 맨 위의 `CHEAT · 모두 최대`로 현재 정당 소속자 전원 최대화

버튼을 누르는 즉시 현재 월드에 적용됩니다. 개인·정당·국가 자금과 행동력의 임의 값 변경은 `config.json`에서 계속 지원합니다. 자체 제작 입력창은 게임 입력 시스템과 충돌하므로 인게임 UI에서 제거했습니다.
